# - Config file for the gz-waves package.
#
# For finding and loading gz-waves from your project, type:
#
# find_package(gz-waves)
#
# This creates the following targets:
#
#   Core library target                     - gz-waves::gz-waves
#   Alternative target name                 - gz-waves::core
#   Core library + all requested components - gz-waves::requested
#
...
#
#   gz-waves_CORE_LIBRARY - Core library (actually contains gz-waves::core)
#   gz-waves_LIBRARIES    - All libraries to link against (actually contains gz-waves::requested).
#   gz-waves_INCLUDE_DIRS - Include directories for gz-waves and its dependencies.
#
# We will also set gz-waves_FOUND to indicate that the package was found.
#
################################################################################
...
if(NOT gz-waves_FIND_QUIETLY)
  message(STATUS "Looking for gz-waves -- found version 1.0.0")
endif()

if(gz-waves_CONFIG_INCLUDED)

  # Check that each of the components requested in this call to find_package(~)
  # have already been found.
  set(gz-waves_all_requested_components_found true)
  foreach(component ${gz-waves_FIND_COMPONENTS})
    if(NOT gz-waves-${component}_FOUND)
      set(gz-waves_all_requested_components_found false)
      break()
    endif()
  endforeach()

  # If this gz-waves-config.cmake file has been called before,
  # and all the requested components are already found, then exit early.
  if(gz-waves_all_requested_components_found)
    return()
  endif()
endif()
set(gz-waves_CONFIG_INCLUDED TRUE)
...
if(NOT gz-waves_FIND_QUIETLY)
  message(STATUS "Searching for dependencies of gz-waves")
endif()
...
if(NOT TARGET gz-waves::gz-waves)
  include("${CMAKE_CURRENT_LIST_DIR}/gz-waves-targets.cmake")

  # Create a simplified imported target name for the core library.
  # You can link to this target instead of the core library.
  add_library(gz-waves::core INTERFACE IMPORTED)
  set_target_properties(gz-waves::core PROPERTIES
    INTERFACE_LINK_LIBRARIES gz-waves::gz-waves)
  ...
  # effectively the same as gz-waves::core, but it does not have the
  # benefit of unambiguously being a target name.
  add_library(gz-waves INTERFACE IMPORTED)
  set_target_properties(gz-waves PROPERTIES
    INTERFACE_LINK_LIBRARIES gz-waves::gz-waves)
...
get_target_property(gz_all_components gz-waves::requested INTERFACE_LINK_LIBRARIES)
if(NOT gz_all_components)
  # If gz-waves::requested has not been given any libraries yet, then cmake will
  ...
  set_target_properties(gz-waves::requested PROPERTIES
    INTERFACE_LINK_LIBRARIES "${gz_all_components};gz-waves::gz-waves")
endif()

# Package variables. Note that gz-waves_LIBRARIES and gz-waves_CORE_LIBRARY
# contain imported targets, so gz-waves_INCLUDE_DIRS is never needed.
set(gz-waves_CORE_LIBRARY gz-waves::core)
set(gz-waves_LIBRARIES gz-waves::requested)

if (NOT gz-waves_INCLUDE_DIRS_OVERRIDE)
  set_and_check(gz-waves_INCLUDE_DIRS "${PACKAGE_PREFIX_DIR}/include/gz/waves1")
else()
  set_and_check(gz-waves_INCLUDE_DIRS ${gz-waves_INCLUDE_DIRS_OVERRIDE})
endif()

# Backwards compatibility variables
set(GZ-WAVES_LIBRARIES ${gz-waves_LIBRARIES})
set(GZ-WAVES_INCLUDE_DIRS ${gz-waves_INCLUDE_DIRS})

# This macro is used by gz-cmake to automatically configure the pkgconfig
# files for Gazebo projects.
gz_pkg_config_entry(gz-waves "gz-waves")

# Find each of the components requested by find_package(~)
foreach(component ${gz-waves_FIND_COMPONENTS})

  if(NOT gz-waves_FIND_QUIETLY)
    message(STATUS "Searching for <gz-waves> component [${component}]")
  endif()

  if(gz-waves_FIND_REQUIRED_${component})
    ...
    # find_dependency(~)
    find_dependency(gz-waves-${component} 1.0.0 EXACT)
    ...
    # forwarded to it.
    find_package(gz-waves-${component} 1.0.0 EXACT ${gz_package_quiet})
...
set(GZ-WAVES_DOXYGEN_TAGFILE "${PACKAGE_PREFIX_DIR}/share/gz/gz-waves/gz-waves.tag.xml")
